...
run_experiment() {
  ...
  LOGDIR="text_logs_sensors/$DATASET/$ALGO/$N_SENSOR/$SEED"
  mkdir -p $LOGDIR
  LOGFILE="$LOGDIR/log.txt"

  echo "Running experiment #$COUNT: Dataset=$DATASET, Alg=$ALGO, N_Sensor= $N_SENSOR, Seed=$SEED on GPU $GPU"
    ...
    while true; do
      for i in "${GPUS[@]}"; do
        ...
        if flock $LOCK_FILE bash -c "[ \$(< $STATUS_DIR/gpu_$i) -lt $PROCESSES_PER_GPU ]"; then
          ...
          run_experiment $SEED $ALGO $DATASET $N_SENSOR $COUNT $i &
          break 2
        fi
      done
...
job_list=()
count=0
for dataset in "Integral" "Derivative"; do
  for algo in "SVD" "Eigen" "matrix" "deeponet" "deeponet_cnn" "deeponet_pod" "deeponet_2stage" "deeponet_2stage_cnn"; do
    for n_sensor in 10 50 100 200 300 400 500 600 700 800 900 1000; do
      for seed in {1..3}; do
        job_list+=("$seed $algo $dataset $n_sensor $count")
        count=$((count + 1))
      done
    done
  done
