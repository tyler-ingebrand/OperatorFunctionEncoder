...
run_experiment() {
  ...
  LOGDIR="text_logs/$DATASET/$ALGO/$N_BASIS/$SEED"
  mkdir -p $LOGDIR
  ...
  python test.py --epochs 30000 --logdir logs_ablation_basis --dataset_type $DATASET --model_type $ALGO --seed $SEED --n_basis $N_BASIS --device $GPU > $LOGFILE 2>&1
