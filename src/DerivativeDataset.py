...
class DerivativeDataset(BaseDataset):
    ...
    def sample(self, device:Union[str, torch.device] ="auto") -> Tuple[ torch.tensor,
        ...
        with torch.no_grad():
            ...
            transformed_ys = 3 * As.unsqueeze(1) * xs ** 2 + 2 * Bs.unsqueeze(1) * xs + Cs.unsqueeze(1)
            ...
            return example_xs, example_ys, xs, transformed_ys, {"As":As, "Bs" : Bs, "Cs": Cs, "Ds": Ds}