...
class SinDataset(BaseDataset):
    ...
    def sample(self, device:Union[str, torch.device] ="auto") -> Tuple[ torch.tensor,
        ...
        with torch.no_grad():
            ...
            # compute the corresponding ys
            # example ys are the true function
            example_ys = As.unsqueeze(1) * torch.sin(example_xs) + Bs.unsqueeze(1) * torch.sin(4*example_xs)  + Cs.unsqueeze(1) * torch.sin(8*example_xs)

            # ys are the transformed function, in this the derivative
            ys = As.unsqueeze(1) * torch.sin(xs) + Bs.unsqueeze(1) * torch.sin(4*xs) + Cs.unsqueeze(1) * torch.sin(8*xs)
...
class CombinedDataset(BaseDataset):
    # maps the quadratic space to the sin space.
    def __init__(self,
                 a_range=(-3, 3),
                 b_range=(-3, 3),
                 c_range=(-3, 3),
                 input_range_source=(-1, 1),
                 input_range_target=(-1, 1),
                 n_functions_per_sample:int = 10,
                 n_examples_per_sample:int = 1_000,
                 n_points_per_sample:int = 10_000,
                 freeze_xs:bool = False,
                 calibration_only:bool = False, # if true, only exmaple data is provided on both datasets. Otherwise, target dataset gets much more data.
                 ):
        super().__init__(input_size=(1,),
                         output_size=(1,),
                         total_n_functions=float('inf'),
        ...
        self.b_range = b_range
        self.c_range = c_range
        self.input_range_source = input_range_source
        self.input_range_target = input_range_target
        self.freeze_xs = freeze_xs
        self.xs = None
        self.calibration = calibration_only
    ...
    def sample(self, device:Union[str, torch.device]) -> Tuple[ torch.tensor,
        ...
        with torch.no_grad():
            ...
            n_functions = self.n_functions_per_sample
            n_examples = self.n_examples_per_sample
            n_points = self.n_points_per_sample if not self.calibration else self.n_examples_per_sample
            ...
            if not self.freeze_xs or self.xs is None:
                example_xs = torch.rand((n_functions, n_examples, *self.input_size), dtype=torch.float32)
                example_xs = example_xs * (self.input_range_source[1] - self.input_range_source[0]) + self.input_range_source[0]
            else:
                example_xs = self.xs
            if self.freeze_xs and self.xs is None:
                self.xs = example_xs
            ...
            # these are never frozen, because deeponet can handle varying output sensor locations.
            xs = torch.rand((n_functions, n_points, *self.input_size), dtype=torch.float32)
            xs = xs * (self.input_range_target[1] - self.input_range_target[0]) + self.input_range_target[0]

            # compute the corresponding ys
            # example data is quadratic, target data is sin
            example_ys = As.unsqueeze(1) * example_xs ** 2 + Bs.unsqueeze(1) * example_xs  + Cs.unsqueeze(1)
            ys = As.unsqueeze(1) * torch.sin(xs) + Bs.unsqueeze(1) * torch.sin(4*xs) + Cs.unsqueeze(1) * torch.sin(8*xs)
